...
// 初始化默认库存数据
const initDefaultInventoryData = () => {
  return [
    {
      id: 1,
      product_id: 1,
...
// 更新库存统计数据
const updateStockStats = (inventoryData) => {
  stockStats.total = inventoryData.length
  stockStats.normal = inventoryData.filter(item => item.stock_status === 'normal').length
  stockStats.warning = inventoryData.filter(item => item.stock_status === 'warning').length
  stockStats.empty = inventoryData.filter(item => item.stock_status === 'empty').length
}
